...
import { COMMUNICATION_CONTRACT, HERDR_LINK_TOOLS, HerdrLinkError } from "../src/protocol.ts";
...
function handlerWith(overrides: McpServerDeps = {}): (message: unknown) => Promise<JsonRpcResponse | null> {
  return createRequestHandler({
    environmentOk: () => true,
    listPeers: () => ok({ self: "brain", peers: ["worker-a"] }),
    sendMessage: () => ok({ status: "sent", id: "hl_unit_1", to: "worker-a" }),
...
test("MCP server request handler", async (t) => {
  await t.test("initialize echoes the client protocol version and reports server info", async () => {
  ...
  await t.test("lists the three canonical tools with schemas when inside Herdr", async () => {
    const listed = await handlerWith()({ jsonrpc: "2.0", id: 1, method: "tools/list" });
    const tools = (listed?.result as { tools: Array<Record<string, unknown>> }).tools;
    assert.deepEqual(tools.map((tool) => tool.name), [...HERDR_LINK_TOOLS]);
  ...
  await t.test("returns peer directory, sent receipt, and close receipt", async () => {
    ...
    assert.deepEqual(JSON.parse(peersResult.content[0]!.text), { self: "brain", peers: ["worker-a"] });
  ...
  await t.test("treats unknown tools and malformed params as protocol-level errors", async () => {
    const handler = createRequestHandler({ environmentOk: () => true });
...
// empty gating, contract declaration, isError passthrough.
// ---------------------------------------------------------------------------
...
/** Canned Herdr CLI used as HERDR_BIN_PATH so the real herdr.ts control layer runs end-to-end. */
function writeFakeHerdrBinary(directory: string): string {
  ...
  writeFileSync(
    ...
    [
      "#!/bin/sh",
      'case "$1 $2" in',
      '  "agent get") printf \'{"result":{"agent":{"name":"brain","pane_id":"w9:p1"}}}\';;',
      '  "agent list") printf \'{"result":{"agents":[{"name":"brain"},{"name":"worker-mcp"}]}}\';;',
      '  "agent prompt")',
      '    if [ "$3" = "ghost" ]; then',
...
test("spawned MCP server over real stdio", async (t) => {
  ...
  await t.test(
    "outside Herdr: handshake works, tools/list is empty, tool call fails as NOT_IN_HERDR (smoke: empty gating)",
    async () => {
  ...
  await t.test(
    "inside Herdr: tools exposed, peers/send/close run against the real control layer, CLI failure surfaces as PEER_NOT_FOUND",
    async () => {
      ...
      try {
        const listed = await server.request({ jsonrpc: "2.0", id: 1, method: "tools/list" });
        const tools = (listed.result as { tools: Array<Record<string, unknown>> }).tools;
        assert.deepEqual(
        ...
        const peersResult = peers.result as { content: Array<{ text: string }>; isError?: boolean };
        assert.notEqual(peersResult.isError, true);
        assert.deepEqual(JSON.parse(peersResult.content[0]!.text), { self: "brain", peers: ["worker-mcp"] });
        ...
        const closed = await server.request({
          ...
          id: 5,
          method: "tools/call",
          params: { name: "herdr_link_close", arguments: { agent: "worker-mcp" } },
        });
        const closedPayload = JSON.parse(
          (closed.result as { content: Array<{ text: string }> }).content[0]!.text,
        ) as Record<string, unknown>;
        assert.deepEqual(closedPayload, { status: "closed", agent: "worker-mcp" });
